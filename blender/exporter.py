...
class XfbinExporter:
    ...
    def make_coords(self, armature: Armature, clump: NuccChunkClump, context) -> List[NuccChunkCoord]:
        ...
        def make_coord_og(bone: EditBone, coord_parent: CoordNode = None, parent_matrix: Matrix = Matrix.Identity(4)):
            ...
            # Recursively add all children of each bone
            for c in bone.children:
                make_coord_og(c, node, Matrix(bone['matrix']))
        ...
        for empty in empties:
            ...
            for mesh_obj in sorted([c for c in empty.children if c.type == 'MESH'], key=lambda x: x.name):
                ...
                for c in empty.children:
                    matlist.append(c.xfbin_mesh_data.xfbin_material)
                
                #a function that counts the number of times a material is used
                def find_duplicates(matlist, mat):
                    count = 0
                    for m in matlist:
                        if mat in m:
                            count += 1
                ...
                if mat is None:
                    self.operator.report(
                        {'WARNING'}, f'[NUD MESH] {mesh_obj.name} in {empty.name} has a non-existing XFBIN material and will be skipped.')
                    continue
                ...
                #if find_duplicates(matlist, mat.name) > 1:
                #check if the material is used more than once
                    self.operator.report(
    ...
    def make_modelhit(self, obj, clump, context):
        ...
        for c in obj.children:
            ...
            vert_sec.flags = ((mesh_data.color[2] + mesh_data.col_flags[2]), (mesh_data.color[1] + mesh_data.col_flags[1]), (mesh_data.color[0] + mesh_data.col_flags[0]))
